...
const DS_PLATFORM_USAGE_URL = "https://platform.deepseek.com/api/v0/usage/cost";
const OR_CREDENTIAL_REF = credentialRef("OPENROUTER_API_KEY");
const DS_CREDENTIAL_REF = credentialRef("DEEPSEEK_API_KEY");
/** Optional platform session token: unlocks official today/month usage from the dashboard API. */
const DS_PLATFORM_TOKEN_REF = credentialRef("DEEPSEEK_PLATFORM_TOKEN");
 ...
 * (the same date-filterable data the platform web console shows). Requires
 * the optional `DEEPSEEK_PLATFORM_TOKEN` credential (localStorage `userToken`
 * of platform.deepseek.com). Returns today's cost in the account currency,
 * or `null` when the token/shape is unavailable.
 */
async function fetchPlatformTodayCost(token) {
  const now = new Date();
  const url = `${DS_PLATFORM_USAGE_URL}?month=${now.getMonth() + 1}&year=${now.getFullYear()}`;
  const response = await fetch(url, {
    headers: {
  ...
  if (body?.code !== 0 || biz === void 0 || biz.biz_code !== 0) {
    ...
    if (code === 40002 || code === 40003) {
      throw new Error("DEEPSEEK_PLATFORM_TOKEN 已过期：请重新登录 platform.deepseek.com 更新 userToken");
    }
...
/** Absolute path of the sessions directory under the harness home. */
function sessionsRoot(ctx) {
  let home;
  const homeFn = typeof ctx.get === "function" ? ctx.get("dshHomePath") : void 0;
  if (typeof homeFn === "function") home = homeFn();
  else if (process.env.DSH_HOME) home = process.env.DSH_HOME;
  else home = join(homedir(), ".dsh");
  return join(home, "sessions");
}

/**
 * Replay EVERY session log and sum DeepSeek (deepseek-official) assistant
 * messages with `time >= sinceMs`, priced with the official table. This
 * covers calls made before the plugin was installed — the ledger cannot.
 * @returns total USD, or `null` when no matching rows were found.
 */
async function replayDeepseekSpend(ctx, sinceMs) {
  const persistence = ctx.get("sessionPersistence");
  if (persistence === void 0 || typeof persistence.readRaw !== "function") return null;
  ...
  try {
    for (const ws of readdirSync(root)) {
      ...
      for (const ent of readdirSync(wsPath)) {
        const p = join(wsPath, ent);
        try {
          if (statSync(p).isDirectory() && existsSync(join(p, "session.jsonl.zstd"))) ids.push(ent);
        } catch {}
      }
    }
  } catch {
  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (dsKey !== null && dsKey !== "") {
        ...
        // Today: official platform figure when the optional token is
        // configured, else a replay of today's session logs (accurate and
        // includes pre-install history). Last hour: replay (fresh window).
        let todaySpend = null;
        let todaySource = "replay";
        try {
          const platformHit = await ctx.credentials.resolve(DS_PLATFORM_TOKEN_REF);
          if (platformHit !== void 0 && platformHit !== null && platformHit.value !== "") {
          ...
          ctx.logger.warn("dsh-quota-dashboard: platform today-cost lookup failed, falling back to replay");
          ctx.logger.warn(error);
        }
        if (todaySpend === null) {
          const replayed = await replayDeepseekSpend(ctx, dayStartMs());
          todaySpend = replayed === null ? null : Math.round(replayed * 1e6) / 1e6;
        }
        const hourReplayed = await replayDeepseekSpend(ctx, Date.now() - 60 * 60 * 1000);
        out.providers.deepseek = {
          ...
          hourSpend: hourReplayed === null ? null : Math.round(hourReplayed * 1e6) / 1e6,
          todaySpend,
          todaySource,
          sessionCost,
          sessionCalls,
